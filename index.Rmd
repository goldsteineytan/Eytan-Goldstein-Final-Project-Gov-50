...
```
```{r}
# Creating graph from coup data that shoes the density of attempts of military involvement in different coup outcomes over time
coup_data |>
  ggplot(aes(x = year)) +
...
mil_boots |>
  ggplot(aes(x = ATE)) +
  geom_histogram(aes(y = after_stat(density)), binwidth = 0.005, fill = "darkseagreen3") + labs(x = "Average Military Treatment Effect", y = "Density", title = "Effect of Military Actors on Coup Realization")
...
mil_ci_95 <- mil_boots |>
  ...
  knitr::kable(col.names = c("Lower Confidence Interval", "Upper Confidence Interval"),digits = 4)
...
#Formatting the multiple regression so that I can see the measurements of the relationships and the p alues for all of the variables
library(stargazer)
